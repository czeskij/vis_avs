...
FFTAnalyzer::FFTAnalyzer(size_t fftSize, size_t bands)
    ...
    , m_bands(bands)
{
    m_ring.resize(m_fftSize * 4, 0.0f); // ring 4x size
    m_window.resize(m_fftSize);
    for (size_t i = 0; i < m_fftSize; ++i) { // Hann
...
bool FFTAnalyzer::platform_fft(const float* time, std::vector<float>& mags)
{
    ...
    if (!g_accel.setup)
        return false;
    // Copy & window into split complex
    ...
    return true;
#else
    // Naive O(n^2) fallback (rarely used):
...
bool FFTAnalyzer::compute(std::vector<float>& outBands)
{
    size_t w = m_writeIdx.load(std::memory_order_relaxed);
    if (w < m_fftSize)
        return false; // wait until buffer reasonably filled initial
    // Grab last m_fftSize samples ending at w
    ...
    // Log-spaced bands between indices [1, N/2)
    double minF = 1.0;
    ...
    for (size_t b = 0; b < m_bands; ++b) {
        double t0 = (double)b / m_bands; // inclusive
        double t1 = (double)(b + 1) / m_bands; // exclusive
        double f0 = logMap(t0);
        double f1 = logMap(t1);
        if (f0 < 1.0)
            f0 = 1.0;
